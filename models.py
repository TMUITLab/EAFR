...
class GCNAlign_GCNConv_relation1(MessagePassing):
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_normal(self.att)
        if self.bias is not None:
            nn.init.zeros_(self.bias)



    def forward(self, x, edge_index, r=None,signs=None, size=None,edge_weight = None):
        ...
        return self.propagate(edge_index, size=size, x=x, r_ij=r,signs = signs,nrm = edge_weight)
